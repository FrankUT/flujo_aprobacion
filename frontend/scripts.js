...
async function obtenerTodasLasSolicitudes() {
    try {
        const url = `${API_BASE_URL}/usuario/${APROBADOR_ID}`;
...
function renderizarSolicitudes(solicitudes) {
    ...
    solicitudes.forEach(sol => {
        ...
        const puedeAccionar = sol.estado === 'pendiente'; // Solo puede aprobar/rechazar si está pendiente
...
/**
 * Llamar al endpoint PUT para cambiar el estado de una solicitud.
 */
async function cambiarEstado(id, nuevoEstado) {
    // ... (El código de confirmación y el fetch PUT es el mismo que el anterior) ...
    if (!confirm(`¿Está seguro de ${nuevoEstado} la solicitud ${id}?`)) {
        return;
    }
    ...
    try {
        ...
        // Obtener todas las solicitudes para refrescar la lista global
